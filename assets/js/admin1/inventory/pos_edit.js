    ...
    $("#submit_btn1").click(function () {
        if ($('#pos_select_form').valid()) {
            ...
            $clone
                ...
                .find('[id="pos-0-item-view"]').attr('id', 'pos-' + ItemIndex + '-item_view').html($('#item_description').val()).end()
                ...
                .find('[name="pos-0-qty"]').attr('name', 'pos-' + ItemIndex + '-qty').attr('id', 'pos-' + ItemIndex + '-qty').attr('onkeyup','set_total('+ItemIndex+')').val($('#qty').val()).end()

                // .find('[id="pos-0-cost-price-view"]').attr('id', 'pos-' + ItemIndex + '-cost_price_view').html($('#cost_price').val()).end()
                .find('[name="pos-0-cost-price"]').attr('name', 'pos-' + ItemIndex + '-cost_price').attr('id', 'pos-' + ItemIndex + '-cost_price').attr('onkeyup','set_total('+ItemIndex+')').val($('#cost_price').val()).end()
...
//                .find('[name="pos-0-tax"]').attr('name', 'pos-' + ItemIndex + '-tax').attr('id', 'pos-' + ItemIndex + '-tax').val($('#tax_type1').val()).end()
                ...
                .find('[name="pos-0-product_discount"]').attr('name', 'pos-' + ItemIndex + '-product_discount').attr('id', 'pos-' + ItemIndex + '-product_discount').attr('onkeyup','set_total('+ItemIndex+')').val($('#product_discount').val()).end()
...
//    get_grand_tot();
    ...
    $('.added_row').each(function () {
        ...
        $(this).find('.removeButton').attr('data-index', ItemIndex2).end()
                .find('.removeButton').attr('onclick', "remove_row(" + ItemIndex2 + ",this);");

        $(this)
            .find('.item_view').attr('id', 'pos-' + ItemIndex2 + '-item_view').end()
            .find('.item_hidden').attr('name', 'pos-' + ItemIndex2 + '-item').attr('id', 'pos-' + ItemIndex2 + '-item').end()

            // .find('.qty_view').attr('id', 'pos-' + ItemIndex2 + '-qty_view').end()
            .find('.qty_hidden').attr('name', 'pos-' + ItemIndex2 + '-qty').attr('id', 'pos-' + ItemIndex2 + '-qty').attr('onkeyup','set_total('+ItemIndex2+')').end()

            // .find('.cost_price_view').attr('id', 'pos-' + ItemIndex2 + '-cost_price_view').end()
            .find('.cost_price_hidden').attr('name', 'pos-' + ItemIndex2 + '-cost_price').attr('id', 'pos-' + ItemIndex2 + '-cost_price').attr('onkeyup','set_total('+ItemIndex2+')').end()

            // .find('.product_discount_view').attr('id', 'pos-' + ItemIndex2 + '-product_discount_view').end()
            .find('.product_discount_hidden').attr('name', 'pos-' + ItemIndex2 + '-product_discount').attr('id', 'pos-' + ItemIndex2 + '-product_discount').attr('onkeyup','set_total('+ItemIndex2+')').end()
...
//            .find('.total_view').attr('id', 'pos-' + ItemIndex2 + '-total_view').end()
//            .find('.product_discount_hidden').attr('name', 'pos-' + ItemIndex2 + '-discount').attr('id', 'pos-' + ItemIndex2 + '-discount').addClass('total_cost').end()
    });
    $('#noof_items').val(ItemIndex2);
}

function set_total(index) {
    qty = $('#pos-' + index + '-qty').val();
    price = $('#pos-' + index + '-cost_price').val();
    discount = $('#pos-'+index+'-product_discount').val();
    console.log(qty,price,discount);
    tot = parseFloat(price) * parseFloat(qty);
    total = parseFloat(tot) - parseFloat(discount);

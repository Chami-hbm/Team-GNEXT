        ...
        $.ajax({
            ...
            success : function (data) {
                ...
                if (!$.trim(data)) {
                    ...
                    $.each(d, function (i, e) {
                        $('#barcode .load_ajax_data').append('<span data-price="' + e.product_customer_price + '" data-qty="'+e.available_stock+'" data-description="' + e.description + '" data-item-id="' + e.product_id + '" data-barcode="'+e.barcode+'" onclick="set_price_when_click_span(this)">' + e.barcode +' / '+ e.description + '</span>');
                    });
                }
            }
        });
...
//    $('#qty_addon').html('Quantity-(Available');
    ...
    $('#item_description').val($(t).attr('data-description'));
